...
from phantom.processors.phantom_data import (
    TrainingData,
    TrainingDataSequence,
    HandSequence,
)
...
class RobotInpaintProcessor(BaseProcessor):
    ...
    def process_one_demo(self, data_sub_folder: str) -> None:
        ...
        sequence, img_overlay, img_birdview = self._process_frames(
            images, data, gripper_actions, gripper_widths
        )
    ...
    def _process_frames(
        ...
        TrainingDataSequence, List[np.ndarray], Optional[List[np.ndarray]]
    ]:
        """
        ...
        for idx in tqdm(
            range(len(images["human_imgs"])), desc="Processing frames"
        ):
            ...
            if frame_results is None:
                ...
                sequence.add_frame(
                    TrainingData.create_empty_frame(
                        frame_idx=idx,
                    )
                )
                ...
                if "birdview" in self.debug_cameras:
                    img_birdview.append(
                        np.zeros_like(images["human_imgs"][idx])
                    )
            else:
    ...
    def _process_single_frame(
        ...
        if self.bimanual_setup == "single_arm":
            ...
            target_state = {
                ...
                "gripper_pos": [
                    right_state.gripper_pos,
                    left_state.gripper_pos,
                ],
            }
        ...
        if self.bimanual_setup == "single_arm":
            ...
            if (
                robot_results["left_pos_err"] > self.TRACKING_ERROR_THRESHOLD
                or robot_results["right_pos_err"]
                > self.TRACKING_ERROR_THRESHOLD
            ):
                logger.warning(
                    f"Tracking error too large at frame {idx}, skipping"
                )
                print(
                    f"Left error: {robot_results['left_pos_err']}, Right error: {robot_results['right_pos_err']}\n"
                )
                return None
        ...
        for cam in self.debug_cameras:
            output[f"{cam}_img"] = (robot_results[f"{cam}_img"] * 255).astype(
                np.uint8
            )
        ...
        if self.skip_existing:
            try:
                ...
                if (
                    str(
                        "video_overlay"
                        + f"_{self.robot}_{self.bimanual_setup}.mkv"
                    )
                    in existing_files
                ):
    ...
    def _load_data(self, paths: Paths) -> Dict[str, np.ndarray]:
        ...
        if self.bimanual_setup == "single_arm":
            ...
            return {
                ...
                "union_indices": np.load(actions_path, allow_pickle=True)[
                    "union_indices"
                ],
            }

        # Load bimanual trajectory data
        smoothed_actions_left_path = (
            str(paths.smoothed_actions_left).split(".npz")[0]
            + f"_{self.bimanual_setup}.npz"
        )
        smoothed_actions_right_path = (
            str(paths.smoothed_actions_right).split(".npz")[0]
            + f"_{self.bimanual_setup}.npz"
        )
        actions_left_path = (
            str(paths.actions_left).split(".npz")[0]
            + f"_{self.bimanual_setup}.npz"
        )
        return {
            ...
            "union_indices": np.load(actions_left_path, allow_pickle=True)[
                "union_indices"
            ],
        }

    def _load_images(
        self, paths: Paths, union_indices: np.ndarray
    ) -> Dict[str, np.ndarray]:
        """
        ...
        return {
            ...
            "human_imgs": np.array(media.read_video(paths.video_human_inpaint))[
                union_indices
            ],
            "imgs_depth": (
                np.load(paths.depth)[union_indices] if self.use_depth else None
            ),
        }

    def _process_gripper_widths(
        self, paths: Paths, data: Dict[str, np.ndarray]
        ...
        return {"left": left_actions, "right": right_actions}, {
            "left": left_widths,
            "right": right_widths,
        }

    def _compute_gripper_actions(
        self, list_gripper_dist: np.ndarray
        ...
        try:
            ...
            min_val, max_val = np.min(list_gripper_dist), np.max(
                list_gripper_dist
            )
    ...
    def _get_robot_state(
        self, ee_pt: np.ndarray, ori_matrix: np.ndarray, gripper_dist: float
    ) -> RobotState:
        """
    ...
    def _process_robot_overlay(
        self, img: np.ndarray, robot_results: Dict[str, Any]
    ) -> np.ndarray:
        """
        ...
        if self.square:
            ...
            rgb_img_sim = cv2.resize(
                rgb_img_sim,
                (int(W / H * self.output_resolution), self.output_resolution),
            )
            robot_mask = cv2.resize(
                robot_results["robot_mask"],
                (int(W / H * self.output_resolution), self.output_resolution),
    ...
    def _process_robot_overlay_with_depth(
        self,
        img: np.ndarray,
        hand_mask: np.ndarray,
        img_depth: np.ndarray,
        robot_results: Dict[str, Any],
    ) -> np.ndarray:
        """
        ...
        if self.square:
            ...
            resize_shape = (
                int(W / H * self.output_resolution),
    ...
    def _save_results(
        ...
        img_birdview: Optional[List[np.ndarray]] = None,
    ) -> None:
        """
        ...
        video_path = (
            str(paths.video_overlay).split(".mkv")[0]
            + f"_{self.robot}_{self.bimanual_setup}.mkv"
        )
        ...
        if img_birdview is not None:
            birdview_path = (
                str(paths.video_birdview).split(".mkv")[0]
                + f"_{self.robot}_{self.bimanual_setup}.mkv"
            )
        ...
        training_data_path = (
            str(paths.training_data).split(".npz")[0]
            + f"_{self.bimanual_setup}.npz"
        )
    ...
    def _save_video(self, path: str, frames: List[np.ndarray]) -> None:
        ...
        """
        media.write_video(
            path, frames, fps=self.DEFAULT_FPS, codec=self.DEFAULT_CODEC
        )
    ...
    def _calculate_sensor_size(
        self, img_w: int, img_h: int, fx: float, fy: float
    ) -> Tuple[float, float]:
        """
    ...
    @staticmethod
    def _convert_real_camera_ori_to_mujoco(
        camera_ori_matrix: np.ndarray,
    ) -> np.ndarray:
        """
